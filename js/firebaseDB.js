...
$(document).ready(async function () {
  ...
  async function UpdateData(userUP) {
    await setDoc(doc(ManagerBehaviosUser, "user_1765904148203_428"), userUP, { merge: true })
      .then(() => {
        console.warn("success")
      })
  }
...
function updateUserStatusOffline() {
    setDoc(doc(ManagerBehaviosUser, userId), {
        TimePauseKey : showResult(),
        status: "offline",
        lastSeen: serverTimestamp()
    }, { merge: true })
    .catch(err => console.error("Lỗi cập nhật offline:", err));
}

// Khi mất mạng tạm thời
window.addEventListener("offline", updateUserStatusOffline);

// Khi đóng tab / reload
window.addEventListener("beforeunload", updateUserStatusOffline);
